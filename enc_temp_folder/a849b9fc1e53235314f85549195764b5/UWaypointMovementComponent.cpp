...
void UUWaypointMovementComponent::BeginPlay()
{
	...
	if (AActor* Owner = GetOwner())
	{
		if (USceneComponent* RC = Owner->GetRootComponent())
		{
			...
			const FTransform Parent = RC->GetComponentTransform(); // world transform del root a t0
			for (FTransform& Tf : Waypoints)
			{
				Tf = Tf.GetRelativeTransform(Parent); // world -> relativo al Root
			}
		}
	}

	if (Waypoints.Num() == 0) { SetComponentTickEnabled(false); return; }
	if (CurrentIndex < 0 || CurrentIndex >= Waypoints.Num()) { CurrentIndex = 0; }
}
